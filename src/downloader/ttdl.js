...
export async function tiktokDl(url) {
  ...
  try {
    ...
    function formatDate(n, locale = "en") {
      // create_time sometimes in seconds — try both
      const milli = n && n.toString().length === 10 ? Number(n) * 1000 : Number(n);
      const d = new Date(milli || Date.now());
    ...
    return {
      status: true,
      title: res.title,
      taken_at: (res.create_time ? formatDate(res.create_time).replace("1970", "") : null),
      region: res.region,
      id: res.id,
      duration: res.duration ? res.duration + " Seconds" : null,
